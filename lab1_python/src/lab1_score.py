...
from lab1_review import Point3D
from lab1_review import find_warning, every_third, all_words_reverse, half_upper_case
...
from lab1_review import rotate_90_y, TailLights, coordinate_transform
from lab1_review import array_to_points, points_to_array, transform_point_array
...
class TestChapter4(unittest.TestCase):
    ...
    def test_all_words_reverse(self):
        message = "The earth is round"
        out = ['round','is','earth','The']
        self.assertEqual( all_words_reverse(message), out )
        message = "Today is a great day!"
        out = ['day!','great','a','is','Today']
        self.assertEqual( all_words_reverse(message), out )
...
class TestChapter9(unittest.TestCase):
    ...
    def test_point3d(self):
        plist = [Point3D(1, 2, 3), Point3D(4, 5, 6)]
        npt.assert_allclose( points_to_array(plist), np.array([[1, 2, 3], [4, 5, 6]]))
        self.assertEqual( array_to_points(np.array([[1, 2, 3], [4, 5, 6]])), plist)

    def test_transform_point_array(self):
        points_array = np.array([[1, 2, 3], [4, 5, 6]])
        r = R.from_euler('xyz', [90, 0, 0], degrees=True)
        t = np.array([10, 20, 30])
        points_array_t = np.array([[11, 17, 32], [14, 14, 35]])
        npt.assert_allclose( transform_point_array(points_array, r, t), points_array_t)
        r = R.from_euler('xyz', [0, 90, 0], degrees=True)
        t = np.array([-10, -20, -30])
        points_array_t = np.array([[-7, -18, -31], [-4, -15, -34]])
        npt.assert_allclose( transform_point_array(points_array, r, t), points_array_t)
...
class TestChapter14(unittest.TestCase):
    ...
    def test_rotate_90_y(self):
        ...
        B = rotate_90_y(A)
        npt.assert_allclose( B, rot_gt, rtol=1e-5, atol=1e-5)


    def test_coordinate_transform(self):
        A = np.reshape(np.arange(12),(4,3))
        gt = np.array([[ 9.66025404,  6.73205081,  1.        ],
                       [11.70929214, 11.37916512, -0.09807621],
                       [13.75833025, 16.02627944, -1.19615242],
                       [15.80736835, 20.67339376, -2.29422863]])
        
        B = coordinate_transform(A)
        npt.assert_allclose( B, gt, rtol=1e-5, atol=1e-5)
