...
class Obstacles(Node):
    ...
    def lidar_centroids(self, msg):    
        ...
        if first_valid_idx == -1:
            ...
            return
        for i in range(num_ranges):
            idx = (first_valid_idx + i) % num_ranges
            next_idx = (first_valid_idx + i + 1) % num_ranges
            if not good[idx]:
                if len(current_cluster_x) > 0:
                    centroids_x.append(np.mean(current_cluster_x))
                    centroids_y.append(np.mean(current_cluster_y))
                current_cluster_x = []
                current_cluster_y = []
                continue
            current_cluster_x.append(x[idx])
            current_cluster_y.append(y[idx])
            if not good[next_idx] or abs(ranges[idx] - ranges[next_idx]) > self.delta_r:
                if len(current_cluster_x) > 0:
                    centroids_x.append(np.mean(current_cluster_x))
                    centroids_y.append(np.mean(current_cluster_y))
                current_cluster_x = []
                current_cluster_y = []
        xcen = np.array(centroids_x)
        ycen = np.array(centroids_y)
