...
class RobotRoom():
    ...
    def draw(self, wait=0):
        ''' Draw the location of the robot in the room
            ...
            Useful for leaders.
            wait: time in ms to wait.  If 0, waits indefinitely
                  If user presses a key in this time, the key is returned
        '''
        img = self.room.copy()
        cv.circle( img, tuple(self.xy), self.radius, self.color, 4)  #Robot is a circle
        cv.imshow(self.name, img)
        return cv.waitKey(wait) & 0xFF

    def draw_no_return_key(self):
        ''' Draw the location of the robot and do not return a key and do not wait
