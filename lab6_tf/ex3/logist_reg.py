...
class LogisticReg:
    ...
    def save_params(self, outfolder):
        os.makedirs(outfolder, exist_ok=True)
        params = dict(cvec = logr.cvec.tolist(),
                      intercept = logr.intercept.tolist())
        with open(os.path.join(args.outfolder, 'logr_coefs.json'), 'w') as f:
            json.dump(params, f)
    ...
    def apply(self, img, mod_channels=False):
        ...
        score = score_flat.reshape(img.shape[0:2])
        return score
    ...
    def find_largest_target(self, prob_target, threshold=0.5, minpix=20):
        ...
        if num_labels < 2:
            return np.array([0, 0]), 0, np.zeros_like(binary_mask)
        areas = stats[1:, cv.CC_STAT_AREA]
        valid_indices = [i + 1 for i, area in enumerate(areas) if area >= minpix]
        if not valid_indices:
            return np.array([0, 0]), 0, np.zeros_like(binary_mask)
        largest_area = 0
        largest_label = -1
        for label_index in valid_indices:
            if stats[label_index, cv.CC_STAT_AREA] > largest_area:
                largest_area = stats[label_index, cv.CC_STAT_AREA]
                largest_label = label_index
        if largest_label == -1:
            return np.array([0, 0]), 0, np.zeros_like(binary_mask)
        centroid = centroids[largest_label]
        area = stats[largest_label, cv.CC_STAT_AREA]
        target_mask = (labels == largest_label).astype(np.uint8)
...
if __name__ == '__main__':
    ...
    if args.outfolder:
        print('Saving params to:',args.outfolder)
        logr.save_params(args.outfolder)
    else:
        print('No outfolder specified.')
    
    if args.testimg is not None:
        # Load test data:
